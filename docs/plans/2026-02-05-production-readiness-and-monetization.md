...
Acceptance:
- [ ] Running any command with bad input returns exit code `2` (not `1`).
- [ ] Running any command with an unexpected exception returns exit code `3`.
...
Acceptance:
- [ ] `mcp-shield <cmd> --json` prints valid JSON and exits with correct code.
- [ ] In `--ci`, prompts never appear; commands fail with clear error if user action required.
...
Acceptance:
- [ ] Add unit tests that validate output against schemas using AJV.
...
- [ ] `npm install && npm run build && npm test` pass on Node 22 and Node 24.
- [ ] `mcp-shield init/add/verify/scan` work end-to-end in a clean temp repo.
- [ ] `verify` blocks drift deterministically (exit 1), with `--json` output.
- [ ] `scan` applies policy thresholds deterministically (exit 1 in CI/policy-block), with `--json` output.
- [ ] Lockfile writes are atomic and stable (no nondeterministic diffs).
- [ ] Artifact extraction is safe (no path traversal / symlink / hardlink overwrite).
- [ ] Dependency vulnerabilities are reported via OSV (at minimum: direct deps).
- [ ] Repo has `LICENSE`, `SECURITY.md`, and `SUPPORT.md`.
- [ ] Release artifacts are published (npm) or there is a complete, working dry-run release pipeline.
...
Files to add:
- [ ] `LICENSE` (MIT, Copyright (c) 2026 Kelly)
- [ ] `SECURITY.md` (vuln reporting instructions + response targets)
- [ ] `SUPPORT.md` (paid support contact, community support policy)
- [ ] `CONTRIBUTING.md` (how to run, test, submit PRs)
- [ ] Fix placeholder links and install instructions:
  - update root `README.md` clone URL + docs links
  - ensure `README.md` documents `npx mcp-shield` as the default install/run path once published

Acceptance:
- [ ] `npm test` still passes.
- [ ] GitHub renders the files correctly.

### PR-002 — Lockfile schema + atomic writes
Implement:
- [ ] Add JSON Schema: `schemas/mcp.lock.schema.json` matching current `LockfileManager` format (`version`, `generatedAt`, `servers{}`).
- [ ] Update `LockfileManager.validate()` to validate using AJV against the schema (keep current checks as a fallback only if schema missing).
- [ ] Implement atomic writes in `LockfileManager.write()`:
  - write to `${lockfilePath}.tmp.<pid>.<rand>`
  - `fsync` (best effort)
  - rename over target
- [ ] Add CLI command: `mcp-shield lock validate [--json] [--ci]`
- [ ] Unify lockfile types:
...
Acceptance:
- [ ] Unit test: write interruption simulation does not corrupt lockfile (best-effort; can test by writing temp then rename).
- [ ] `mcp-shield lock validate --json` outputs schema validation results.
...
- [ ] Update `init` policy template URL to a real local doc: `docs/policy-yaml-spec.md` (create if missing).

2) Implement policy loader:
- [ ] New core module: `packages/core/src/policy.ts`
...
- [ ] `scan`:
  - return exit `1` if any result violates policy (in `--ci` or if policy has `enforce: true`)

Policy schema (implement exactly):
```yaml
version: "1.0"
global:
  enforce: false
  ...
  maxRiskScore: 50
  blockVerdicts: ["malicious", "suspicious"]
  maxArtifactSizeBytes: 52428800  # 50 MiB
servers: {}
```

Acceptance:
- [ ] Add tests for each policy field.
- [ ] Running `scan` with a forced `riskScore=99` fixture fails in `--ci`.

### PR-004 — CLI contract plumbing (`--json`, `--ci`, exit codes)
Implement:
- [ ] Add commander global options in `packages/cli/src/cli.ts` and thread options into commands.
- [ ] Create `packages/cli/src/output.ts` with:
...
Acceptance:
- [ ] Snapshot tests for JSON outputs (stable ordering).
- [ ] `--json` produces no ANSI and no extra stdout.

### PR-005 — NPM resolver hardening (network + redirects + timeouts)
Implement in `packages/core/src/artifact-resolver.ts`:
- [ ] Switch npm registry fetch + tarball download to `got` (already a dependency in core).
- [ ] Enforce:
  - timeout: 15s connect + 60s request (configurable via env)
  - max redirects: 3
  - max artifact size: policy global `maxArtifactSizeBytes` (default 50 MiB)
- [ ] Add `--offline` mode:
  - `verify` and `scan` must refuse network and only use cache
  - error message must include how to disable offline
- [ ] Add test dependency and mocks:
...
Acceptance:
- [ ] Add tests that simulate 302 redirect and validate download succeeds.
- [ ] Add tests that simulate oversize artifact and validate hard fail.

### PR-006 — Cache: move to OS cache dir + CLI commands
Implement:
- [ ] Update `CacheManager` default dir:
...
- [ ] Add CLI: `mcp-shield cache gc [--max-age-days N]` and `mcp-shield cache purge`

Acceptance:
- [ ] E2E test: cache persists across runs in a temp home dir fixture.

### PR-007 — Scanner safety + dependency vuln integration (OSV)
This is a **must** for production.

1) Dependency security:
- [ ] Add OSV client in core: `packages/core/src/osv.ts`
...
2) Scanner hardening:
- [ ] Upgrade `tar` dependency in `packages/scanner/package.json` to a patched version (>= 7.5.3). (As of Feb 2026 there is a known `tar` extraction vulnerability fixed in `tar@7.5.3`.)
- [ ] Extract with explicit secure options:
  - `preservePaths: false`
  - `unlink: true`
  - `strict: true`
...
3) Findings taxonomy:
- [ ] Create `packages/scanner/src/rules.ts` exporting rule IDs and default severities.
- [ ] Update all findings to include `details.ruleId`.

Acceptance:
- [ ] Add a regression test that attempts to scan a crafted tarball containing `../evil` and ensure it is rejected and reported as a critical finding.
- [ ] Add a test that OSV integration returns at least one vuln for a known vulnerable package fixture (use recorded HTTP via nock or local fixture JSON).

### PR-008 — SARIF output + GitHub Code Scanning integration
Implement:
- [ ] Add `--sarif` flag to `scan`:
...
- [ ] Add GitHub Action workflow: `.github/workflows/code-scanning.yml`:
...
Implement:
- [ ] Use npm “trusted publishing” (OIDC) for `@mcpshield/core`, `@mcpshield/scanner`, `@mcpshield/cli`
- [ ] Add `.github/workflows/release.yml`:
  - only on tags `v*`
  - `permissions: id-token: write, contents: read`
  - `npm publish` for each workspace package
...
Implement:
- [ ] `docs/PRICING.md` with Pilot pricing (per-repo/month) + what’s included.
- [ ] `docs/ROADMAP.md` with next 90 days (Pro candidates).
- [ ] `docs/SECURITY-POSTURE.md` with threat model summary and non-goals.

Acceptance:
- [ ] Docs site links to these pages.
...
- [ ] Add OS smoke tests (Pilot):
  - Linux required
  - macOS optional (but recommended) as a separate job

Acceptance:
- [ ] CI passes on Node 22 and Node 24.
- [ ] No workflow step hits the network in unit tests (E2E may, but must be labeled and allowed to fail if flaky).

### PR-012 — Support tooling (`doctor`, debug logs)
Implement:
- [ ] Add CLI command: `mcp-shield doctor [--json] [--ci]`
...
Acceptance:
- [ ] `mcp-shield doctor --json` validates against a schema `schemas/cli/doctor-output.schema.json`.
...
- [ ] In a temp dir:
  - [ ] `mcp-shield init`
  - [ ] `mcp-shield add io.github.modelcontextprotocol/filesystem -y` (or a known registry server)
  - [ ] `mcp-shield verify --ci --json`
  - [ ] `mcp-shield scan --ci --json`
